...
async function expandSequence(tasks, options) {
  const results = []
  await parallelize(tasks.map((task, index) => async () =>
    results[index] = await expandTasks(task, options)))
  return results.reduce((result, task) => {
    if (task) {
      if (Array.isArray(task)) result.push(...task)
...
async function expandTasks(config, options = {}, parallelize) {
  if (!config) return config
  if (Array.isArray(config)) return expandSequence(config, options)
  if (typeof config === 'object') {
    if (config.parallel !== undefined) {
      const { tasks } = config
      if (Array.isArray(tasks)) {
        config.tasks = await expandSequence(tasks, options)
        return config
      }
